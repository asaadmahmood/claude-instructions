...
description: Elite frontend image-direction skill for generating premium, conversion-aware website design references. CRITICAL OUTPUT RULE — generate ONE separate horizontal image FOR EVERY section. A landing page with 8 sections produces 8 images. Never compress multiple sections into one image. Enforces composition variety (not always left-text / right-image), background-image freedom, varied CTAs, varied hero scales (giant / mid / mini minimalist), narrative concept spine, second-read moments, and a single consistent palette across all images. Optimized for landing pages, marketing sites, and product comps that developers or coding models can accurately recreate. For five alternate hero-only concepts of the same brand, use `hero-sections.md` instead.
---
...
- the first section image should include site header / top navigation unless the user explicitly says otherwise
...
Based on this skill, generate images for a multi-section website: **exactly one separate horizontal image per section** (not a collage, not one tall page). If the user specifies N sections, output N images. If the count is ambiguous, use §5 defaults (e.g. landing page → 6 sections / 6 images; full website or marketing site → 8).

The design should feel polished: conversion-aware, with striking visuals and creative layouts where the brief allows, without collapsing multiple sections into one frame.

Order images in scroll order. The **first** image is the hero (include site header / top navigation unless the brief says otherwise). The **last** image is the final CTA, or CTA + footer when the section list implies a full page.

**Five hero-only directions for the same brand** (no full page): use `design-prompts/hero-sections.md` instead of this file.

### Type of site

Paste the site brief, brand, section count (if any), and constraints in the **same message** after this block. Use it as source of truth for:

- brand / product name
- what the product is (industry, category, audience)
- number of sections (if specified; else apply §5 defaults)
- primary accent color and palette
- background tone (light / dark / off-white / etc.)
- typography mood
- required brand elements (logo, nav, grid, textures, motifs)
- tone of voice (technical, editorial, playful, enterprise, etc.)
- continuity (e.g. "match the previous run")

Paste your brief below in the same message.
